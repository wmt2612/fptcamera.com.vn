jQuery(document).ready(function($) {
  ...
  function call_ajax() { 
    ...
    if(data.length >= 2){
      $.ajax({
        ...
        beforeSend: function () {
          $('.search-ajax').addClass('loading');
        },
        success: function (data) {
            $("#load-data").css("display", "block");
            $('.search-ajax').removeClass('loading');
            $('#load-data').html(data);
        }
      });
    }else{          
  ...
  function call_ajax_mobile() { 
    var data = $('.search-ajax-mobile').val(); 
    if(data.length >= 2){
      $.ajax({
        url: ajax.url,
        type: 'POST',
        ...
        beforeSend: function () {
          $('.search-ajax-mobile').addClass('loading');
        },
        success: function (data) {
          $("#load-data-mobile").css("display", "block");
          $('.search-ajax-mobile').removeClass('loading');
          $('#load-data-mobile').html(data);
        }
      });
    }else{          
