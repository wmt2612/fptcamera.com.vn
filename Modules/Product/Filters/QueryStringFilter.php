...
class QueryStringFilter
{
    ...
    public function alphabeticDesc($query)
    {
        $query->join('product_translations', function (JoinClause $join) {
            $join->on('products.id', '=', 'product_translations.product_id');
        })
        // ->groupBy($this->groupColumns)
        ->orderByDesc('product_translations.name');
    }
    ...
    {
        $query->where('price', '>=', $this->convertPrice($price));
    }
    ...
    public function bestSale($query)
    {
        $query->selectRaw('SUM(order_products.qty) as total')
            ->leftJoin('order_products', function (JoinClause $join) {
