...
@endpush
<script src="//cdn.ckeditor.com/4.20.1/full-all/ckeditor.js"></script>
<script>
       	CKEDITOR.plugins.addExternal('contents', '{{ Theme::url("assets/js/ckeditor/plugins/toc/plugin.js") }}');
        ...
        if ( dialog.name == 'image' ) {
                // Get dialog we want
                let def = evt.data.definition;

                //Get The Desired Tab Element
                let infoTab = def.getContents( 'info' );
                //Add Our Button
                infoTab.add( {
                type: 'button',
                id: 'chooseImageFromMediaBtn',
                label: 'Choose Image From Media',
                title: 'Upload',
...
<script>
        function pickImage(e, infoTab) {
                let picker = new MediaPicker({ type: 'image' });
                let filePath = '';
                picker.on('select', (file) => {
                        $('input.cke_dialog_ui_input_text').first().val(file.path);
                });
        }
