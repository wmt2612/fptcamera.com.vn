...
class GoogleSheetComment
{
    ...
    {
        $this->spreadSheetId = config('services.google_sheet_adsen.id');
    ...
    public function readGoogleSheet()
    {
        ...
        $range = 'Comment!A1:' . $dimensions['colCount'];
    ...
    public function saveDataToSheet(array $data)
    {
        ...
        $range = "Comment!A" . ($dimensions['rowCount'] + 1);
    ...
    private function getDimensions($spreadSheetId)
    {
        $rowDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'Comment!A:A', 'majorDimension' => 'COLUMNS']
        );
        ...
        $colDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'Comment!1:1', 'majorDimension' => 'ROWS']
        );
