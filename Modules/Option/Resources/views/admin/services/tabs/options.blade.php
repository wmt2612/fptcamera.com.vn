...
@push('globals')
<script>
    FleetCart.data['service.options'] = {!! old_json('options', $service->options) !!};
    FleetCart.errors['service.options'] = @json($errors->get('options.*'), JSON_FORCE_OBJECT);
</script>
@endpush

@include('option::admin.options.templates.service_option', compact('service'))
