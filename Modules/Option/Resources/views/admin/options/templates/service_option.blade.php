<script type="text/html" id="option-service-template">
        ...
        <div class="panel panel-default option">
            <div class="panel-heading">
                <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#option-<%- optionId %>"
                        href="#custom-collapse-<%- optionId %>">
                    ...
                    <div class="new-option clearfix">
                        ...
                        <div class="col-lg-6 col-md-12 p-l-0">
                            <div class="form-group">
                                ...
                                <input type="text" name="options[<%- optionId %>][name]"
                                    class="form-control option-name-field" id="option-<%- optionId %>-name"
                                    value="<%- option.name %>">
                                {{-- <select name="options[<%- optionId %>][name]"
                                class="form-control option-name-field" id="option-<%- optionId %>-name"
                                value="<%- option.name %>">
                                </select> --}}
                            </div>
                        ...
                        <div class="col-lg-3 col-md-12 p-l-0">
                            <div class="form-group">
                                ...
                                <select name="options[<%- optionId %>][type]" class="form-control custom-select-black"
                                    id="option-<%- optionId %>-type">
                                    <option value="" <%= option.type === null ? 'selected' : '' %>>
                                        {{ trans('option::options.form.option_types.please_select') }}
                                    </option>

                                    <optgroup label="{{ trans('option::options.form.option_types.text') }}">
                                        <option value="field" <%= option.type === 'field' ? 'selected' : 'selected' %>>
                        ...
                        <div class="checkbox">
                            ...
                            <input type="checkbox" name="options[<%- optionId %>][is_required]" class="form-control"
                                id="option-<%- optionId %>-is-required" value="1"
                                <%= option.is_required ? 'checked' : '' %>>

                            <label
                                for="option-<%- optionId %>-is-required">{{ trans('option::attributes.is_required') }}</label>
                        </div>

                        <button type="button" class="btn btn-default delete-option pull-right" data-toggle="tooltip"
